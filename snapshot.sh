...
function usage() {
  echo "Usage: "
  echo "$BASH_SOURCE <ACTION=backup> <TAG_VALUE> <AWS_PROFILE>" 
  echo "$BASH_SOURCE <ACTION=delete> <TAG_VALUE> <AWS_PROFILE> <DAYS>" 
}
...
function snapshot_ebs () {
  ...
  do  
    ...
    do
      ...
      if [ "$?" -ne 0 ]; then
        logger "[ERROR] :  Failed to create snapshot for instance=$instance volume=$volume tag_value=$TAG_VALUE"
      fi
    done
  done
}
